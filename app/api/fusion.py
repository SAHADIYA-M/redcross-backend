...
def resolve_all_fusion_candidates(
    repository: Annotated[FusionRepository, Depends(get_fusion_repository)],
    service: Annotated[FusionService, Depends(get_fusion_service)],
) -> dict:
    """Resolve all pending fusion candidates in bulk."""
    candidates = repository.get_all()
    count = 0
    for cand in candidates:
        if cand.status == FusionStatus.PENDING:
            service.resolve(cand.id, FusionResolution.DISMISSED)
            count += 1
    return {"success": True, "count": count, "message": f"Successfully resolved {count} items"}
